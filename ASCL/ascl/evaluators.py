...
def extract_all_features(model, data_loader, print_freq=50):
    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, camid, idx) in enumerate(data_loader):
            data_time.update(time.time() - end)
...
def evaluate_all(query_features, gallery_features, distmat, query=None, gallery=None,
    ...
    if query is not None and gallery is not None:
        ...
        # zdy
        query_frame = [frame for frame, _, _ in query]
        gallery_frame = [frame for frame, _, _ in gallery]
    else:
    ...
    mAP = mean_ap(distmat, query_ids, gallery_ids, query_cams, gallery_cams, query_frame, gallery_frame)
