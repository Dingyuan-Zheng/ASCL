...
class InterCamProxy(nn.Module):
    ...
    def forward(self, inputs, targets, cams):
        ...
        for i in range(B):
            pos_mask = (targets[i] == self.pids).float() * (cams[i] != self.cids).float() # ori: only inter- cam positive
            #pos_mask = (targets[i] == self.pids).float() # zdy: both intra- and inter- cam positive
