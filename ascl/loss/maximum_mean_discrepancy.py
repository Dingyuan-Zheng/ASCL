...
class MaximumMeanDiscrepancy(nn.Module):
    ...
    def __init__(self, use_gpu=True, batch_size=64, instances=4, distance_only=True, all=False):
    ...
    # def forward(self, source_features, target_features, tri_s, tri_t):
    #
    #     # We want to modify only target distribution
    #     trit = tri_t
    #     tris = tri_s.detach()
    #
    #     return self.mmd_loss(source_features, target_features), self.mmd_loss(tris, trit)

    def forward(self, tri_s, tri_t):

        # We want to modify only target distribution
        trit = tri_t
        tris = tri_s.detach()

        return  self.mmd_loss(tris, trit)
