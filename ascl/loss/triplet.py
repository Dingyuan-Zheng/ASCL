...
class SoftTripletLoss(nn.Module):
	...
	def forward(self, emb, label):
		...
		#dist_ap, dist_an, ap_idx, an_idx = _batch_hard(mat_dist, mat_sim, 0, indice=True)
		dist_ap, dist_an, ap_idx, an_idx = _batch_hard(mat_dist, mat_sim, indice=True)
		assert dist_an.size(0) == dist_ap.size(0)
		triple_dist = torch.stack((dist_ap, dist_an), dim=1)
		triple_dist = F.log_softmax(triple_dist, dim=1)
		loss_1 = (- self.margin * triple_dist[:, 0] - (1 - self.margin) * triple_dist[:, 1]).mean()

		# dist_ap, dist_an, ap_idx, an_idx = _batch_hard(mat_dist, mat_sim, 3, indice=True)
		# assert dist_an.size(0) == dist_ap.size(0)
		# triple_dist = torch.stack((dist_ap, dist_an), dim=1)
		# triple_dist = F.log_softmax(triple_dist, dim=1)
		# loss_2 = (- self.margin * triple_dist[:, 0] - (1 - self.margin) * triple_dist[:, 1]).mean()
		#
